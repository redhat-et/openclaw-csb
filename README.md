...
| **CSB** *(multi-arch)* | RHEL AI base (aarch64 + x86_64) | `:csb-latest` | `entrypoint.sh` | RHEL AI base, auto-resolves architecture on pull |
...
Every pushed image is scanned by [Trivy](https://github.com/aquasecurity/trivy) for known vulnerabilities (HIGH and CRITICAL severity), with results archived as workflow artifacts (retained 90 days) in JSON and SARIF formats.

Every pushed image also gets an SBOM generated by [syft](https://github.com/anchore/syft) in both industry-standard formats, attached to the workflow run as a downloadable artifact (retained 90 days):
