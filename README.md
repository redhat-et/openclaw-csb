...
| **CSB (aarch64)** | RHEL AI pre-built (aarch64) | `:csb-aarch64-latest` | `entrypoint.sh` | ARM64 deployments, RHEL AI base, enterprise aarch64 workloads |
| **CSB (x86_64)** | RHEL AI pre-built (x86_64) | `:csb-x86_64-latest` | `entrypoint.sh` | Standard x86_64 deployments on RHEL AI base |
...
**Required secrets:** `QUAY_USERNAME` (redhat-et org robot account), `QUAY_PASSWORD` (robot account token), `GITLAB_REGISTRY_USERNAME` and `GITLAB_REGISTRY_TOKEN` (for pulling the CSB base image from `registry.gitlab.com`).

### Tags

| Purpose | UBI 10 tag | Hummingbird tag | CSB aarch64 tag | CSB x86_64 tag |
|---|---|---|---|---|
| Latest stable | `:latest` | `:hummingbird-latest` | `:csb-aarch64-latest` | `:csb-x86_64-latest` |
| Dated build | `:YYYY.MM.DD` | `:hummingbird-YYYY.MM.DD` | `:csb-aarch64-YYYY.MM.DD` | `:csb-x86_64-YYYY.MM.DD` |
| Immutable git SHA | `:git-<sha>` | `:hummingbird-git-<sha>` | `:csb-aarch64-git-<sha>` | `:csb-x86_64-git-<sha>` |
| Tracks upstream | `:openclaw-<version>` | `:hummingbird-openclaw-<version>` | `:csb-aarch64-openclaw-<version>` | `:csb-x86_64-openclaw-<version>` |

### Software Bill of Materials (SBOM)

Every pushed image is scanned by [Trivy](https://github.com/aquasecurity/trivy) for known vulnerabilities (HIGH and CRITICAL severity), with results archived as workflow artifacts (`trivy-ubi10` / `trivy-hummingbird` / `trivy-csb-aarch64` / `trivy-csb-x86_64`, retained 90 days) in JSON and SARIF formats.

Every pushed image also gets an SBOM generated by [syft](https://github.com/anchore/syft) in both industry-standard formats, attached to the workflow run as a downloadable artifact (`sbom-ubi10` / `sbom-hummingbird` / `sbom-csb-aarch64` / `sbom-csb-x86_64`, retained 90 days):
